...
test('النطاق closed() يشمل كل مغلق معتمدًا كان أو لا (قرار T06)', function (): void {
    expect(Beneficiary::closed()->pluck('id')->sort()->values()->all())
        ->toBe(collect([$this->pendingClosed->id, $this->approvedClosed->id])->sort()->values()->all());
});
